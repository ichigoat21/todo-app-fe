...
export const Dashboard = () => {
  ...
  // @ts-ignore - todo is not typed
  const [todo, setTodo] = useState([]);
  const [value, setValue] = useState("");
  const [editIndex, setEditIndex] = useState<number | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  // @ts-ignore - username not typed
  ...
  useEffect(() => {
    async function fetchTodos() {
      try {
        ...
        // @ts-ignore - username possibly undefined or any
  ...
  async function addOrUpdateTodo() {
    ...
    if (editIndex !== null) {
      // @ts-ignore - todo is untyped
      const updatedTodo = [...todo];
      // @ts-ignore - _id possibly missing
      ...
      if (response.data.todo) {
        // @ts-ignore - setTodo expects typed array
